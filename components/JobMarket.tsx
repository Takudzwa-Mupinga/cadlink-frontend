...
const JobMarket: React.FC<JobMarketProps> = ({ onStartProject }) => {
  ...
  const sortedJobs = showRecsOnly
    ? [...filteredJobs].sort((a, b) => {
        const scoreA =
          recommendations.find((r) => r.jobId === a.id)?.score || 0;
        const scoreB =
          recommendations.find((r) => r.jobId === b.id)?.score || 0;
  ...
  const handleGetRecommendations = async () => {
    ...
    const recs = await recommendJobs(
      currentUser.skills,
      currentUser.role,
      currentUser.pastActivity,
      jobs,
    );
  ...
  const handlePostJob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://backend-2xp1.onrender.com/api/jobs", {
        method: "POST",
        headers: {
  ...
  const handleEnhanceDescription = async () => {
    ...
    const enhanced = await enhanceServiceDescription(
  ...
  useEffect(() => {
    const loadJobs = async () => {
      try {
        ...
        const response = await fetch("https://backend-2xp1.onrender.com/api/jobs/open");
        ...
        const mappedJobs: Job[] = apiJobs.map((job: any, index: number) => ({
          id: job.id ?? `job-${index}`,
          title: job.title,
          client: job.client ?? "Marketplace Client",
          type: job.type,
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-cad-muted mt-1 text-sm">
              Access premium contracts and specialized talent.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            <div className="flex items-center gap-1 bg-cad-panel p-1 rounded-xl border border-cad-border flex-1 md:flex-none">
              <button
                ...
                className={`flex-1 md:flex-none text-center px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${viewMode === "jobs" ? "bg-cad-accent text-cad-dark shadow-sm" : "text-slate-500 hover:text-cad-text hover:bg-cad-surface"}`}
              >
                Contracts
              </button>
              <button
                onClick={() => setViewMode("services")}
                className={`flex-1 md:flex-none text-center px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${viewMode === "services" ? "bg-cad-accent text-cad-dark shadow-sm" : "text-slate-500 hover:text-cad-text hover:bg-cad-surface"}`}
              >
                Talent
              </button>
            </div>
            ...
            >
              ...
              {viewMode === "jobs" ? "Post Contract" : "List Service"}
            </button>
          </div>
        </div>
          ...
          <div className="glass-panel p-1 rounded-2xl bg-gradient-to-r from-indigo-500/20 to-blue-600/20 border border-indigo-500/30">
            <div className="bg-cad-panel rounded-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden backdrop-blur-xl">
              <div className="relative z-10 flex gap-5 items-center">
                ...
                <div>
                  <h3 className="font-bold text-cad-text text-lg">
                    AI Matching Engine
                  </h3>
                  <p className="text-cad-muted text-sm mt-1">
                    We found{" "}
                    <span className="text-cad-text font-bold">3 contracts</span>{" "}
              ...
              <button
                ...
                className="w-full md:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/30 relative z-10 flex items-center justify-center gap-2 active:scale-95"
              >
                {isLoadingRecs ? (
        ...
        <div className="glass-panel p-2 rounded-xl flex flex-col md:flex-row items-center gap-3 sticky top-4 z-20 shadow-2xl backdrop-blur-xl border border-cad-border bg-cad-panel/80">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-bold transition-colors ${showFilters ? "bg-cad-surface text-cad-text" : "text-slate-400 hover:text-cad-text"}`}
            >
          ...
          {viewMode === "jobs" ? (
            loadingJobs ? (
              ...
              sortedJobs.map((job) => (
                <div
                  ...
                  className="group glass-card rounded-2xl p-8 hover:bg-cad-surface transition-all border border-cad-border hover:border-cad-accent/30 cursor-pointer relative overflow-hidden shadow-sm hover:shadow-lg"
                >
                  {/* Hover Glow */}
                  ...
                  <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="hidden md:block text-2xl font-bold text-cad-text group-hover:text-cad-accent transition-colors mb-2 leading-tight">
                        {job.title}
                      </h3>
                      ...
                      <p className="text-slate-400 text-sm leading-relaxed max-w-4xl line-clamp-2 mb-6">
                        {job.description}
                      </p>
                      <div className="flex gap-2">
                        {job.software.map((s) => (
                          <span
                            key={s}
                            className="text-[10px] font-bold text-white bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 group-hover:border-white/20 transition-colors"
                          >
                            {s}
                          </span>
                        ))}
                    ...
                    <div className="md:hidden w-full border-t border-cad-border pt-4 mt-2 flex justify-between items-center">
                      <p className="text-xs text-cad-muted font-bold uppercase tracking-wider">
                        {job.experienceLevel}
                      </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredServices.map((service) => (
                <div
                  key={service.id}
                  className="glass-card rounded-2xl overflow-hidden hover:border-cad-accent/30 transition-all group cursor-pointer border border-cad-border hover:-translate-y-1"
                >
                  ...
                  <div className="p-5">
                    <h3 className="text-cad-text font-bold text-sm line-clamp-2 mb-3 h-10 leading-snug">
                      {service.title}
                    </h3>
                    <div className="flex justify-between items-center border-t border-cad-border pt-3">
                      ...
                      <p className="text-cad-text font-bold text-sm">
                        From ${service.price}
                      </p>
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-3xl bg-cad-panel rounded-3xl border border-cad-border shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
              ...
              <div className="p-6 border-b border-cad-border flex justify-between items-start bg-cad-surface">
                <div>
                  <h3 className="text-2xl font-bold text-cad-text mb-1">
                    {selectedJob.title}
                  </h3>
                ...
                <button
                  ...
                  className="text-slate-500 hover:text-cad-text p-2 hover:bg-cad-border rounded-full transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <div className="prose prose-invert max-w-none mb-8">
                  ...
                  <p className="text-cad-muted text-sm leading-relaxed">
                    {selectedJob.description}
                  </p>
                </div>

                <div className="bg-cad-surface rounded-2xl p-6 border border-cad-border mb-8">
                  <h4 className="text-cad-text font-bold mb-4 text-sm uppercase tracking-wider">
                    Submit Proposal
                  </h4>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        type="number"
                        className="w-full bg-cad-panel border border-cad-border rounded-xl px-4 py-3 text-cad-text text-sm focus:border-cad-accent outline-none"
                        placeholder="1000"
                        value={bidRate}
                  ...
                  <div className="mb-4">
                    <label className="text-xs font-bold text-slate-500 block mb-1.5 flex justify-between">
                      ...
                      <button
                        ...
                        className="text-cad-accent hover:text-cad-text transition-colors flex items-center gap-1"
                      >
                        {isGeneratingAi ? (
                    ...
                    <textarea
                      rows={5}
                      className="w-full bg-cad-panel border border-cad-border rounded-xl p-4 text-cad-text text-sm focus:border-cad-accent outline-none leading-relaxed"
                      value={coverLetter}
                  ...
                  {bidRate && (
                    <div className="bg-cad-panel rounded-xl p-4 border border-cad-border space-y-2">
                      ...
                      <div className="flex justify-between text-sm text-cad-muted">
                        ...
                        <span>-${(parseFloat(bidRate) * 0.1).toFixed(2)}</span>
                      </div>
                      ...
                      <div className="flex justify-between text-base font-bold text-cad-text">
                        ...
                        <span className="text-cad-success">
                          ${(parseFloat(bidRate) * 0.9).toFixed(2)}
                        </span>
                      </div>
                    </div>
                  )}
              ...
              <div className="p-6 border-t border-cad-border bg-cad-surface flex justify-between items-center">
                <button
                  ...
                  className="text-sm font-bold text-cad-muted hover:text-cad-text flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-cad-panel transition-colors"
                >
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setSelectedJob(null)}
                    className="px-5 py-2.5 rounded-xl text-sm font-bold text-cad-muted hover:text-cad-text hover:bg-cad-panel"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={handleApply}
                    className="px-8 py-2.5 bg-cad-accent text-cad-dark rounded-xl text-sm font-bold hover:bg-violet-400 shadow-lg shadow-cad-accent/20 active:scale-95 transition-all"
                  >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-2xl bg-cad-panel rounded-3xl border border-cad-border shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95">
              <div className="p-6 border-b border-cad-border flex justify-between items-center bg-cad-surface">
                <h3 className="text-xl font-bold text-cad-text">
                  Post a New Job/Contract
                </h3>
                <button
                  onClick={() => setIsPostingJob(false)}
                  className="text-slate-500 hover:text-cad-text"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              <form
                ...
                className="p-8 space-y-5 overflow-y-auto max-h-[80vh] custom-scrollbar"
              >
                <div>
                  ...
                  <input
                    required
                    type="text"
                    placeholder="e.g. Mechanical Design for HVAC System"
                ...
                <div>
                  ...
                  <textarea
                    required
                    rows={5}
                    placeholder="Describe the project deliverables, timeline, and requirements..."
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-cad-panel border border-cad-border rounded-xl px-4 py-3 text-cad-text focus:border-cad-accent outline-none"
                ...
                <div>
                  ...
                  <select
                    className="w-full bg-cad-panel border border-cad-border rounded-xl px-4 py-3 text-cad-text"
                ...
                <div className="pt-4 border-t border-cad-border flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setIsPostingJob(false)}
                    className="px-6 py-3 rounded-xl font-bold text-slate-400 hover:text-cad-text transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-8 py-3 bg-cad-accent text-cad-dark rounded-xl font-bold hover:bg-violet-400 shadow-lg shadow-cad-accent/20"
                  >
                    Post
                  </button>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      required
                      type="text"
                      placeholder="do high quality 3D rendering..."
                      ...
                      value={newService.title?.replace("I will ", "")}
                      onChange={(e) =>
                        setNewService({
                          ...newService,
                          title: `I will ${e.target.value}`,
                        })
                      }
                    />
                  </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-cad-muted uppercase tracking-wider mb-2 flex justify-between">
                    ...
                    <button
                      ...
                      className="text-cad-accent hover:text-cad-text transition-colors flex items-center gap-1 text-[10px]"
                    >
                      {isEnhancingDesc ? (
                        <Loader2 className="w-3 h-3 animate-spin" />
                      ) : (
