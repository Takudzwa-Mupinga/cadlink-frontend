...
const Messages: React.FC<MessagesProps> = ({ initialUserId, onClearInitialUser, onStartCall }) => {
    ...
    return (
        <div className="h-full flex flex-col md:flex-row bg-[#0B1121] overflow-hidden">
            {/* LEFT: Conversation List */}
            <div className={`w-full md:w-80 lg:w-96 border-r border-white/5 bg-[#0f1423] flex flex-col ${activeConvId ? 'hidden md:flex' : 'flex'}`}>
                <div className="p-6 border-b border-white/5">
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-900/50 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-sm text-white focus:border-cad-accent outline-none font-medium"
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {filteredConversations.map(conv => (
                        <div 
                            ...
                            className={`p-4 flex items-start gap-3 cursor-pointer transition-colors border-b border-white/5 hover:bg-white/5 ${activeConvId === conv.id ? 'bg-white/5 border-l-2 border-l-cad-accent' : 'border-l-2 border-l-transparent'}`}
                        >
                            <div className="relative">
                                <img src={conv.userAvatar} alt={conv.userName} className="w-12 h-12 rounded-xl object-cover border border-white/10" />
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline mb-1">
                                    <h4 className={`text-sm truncate pr-2 ${conv.unreadCount > 0 ? 'font-bold text-white' : 'font-medium text-slate-200'}`}>{conv.userName}</h4>
                                    <span className="text-[10px] text-slate-500">{conv.lastMessageTime}</span>
                                </div>
                                <p className={`text-xs truncate ${conv.unreadCount > 0 ? 'text-white font-medium' : 'text-slate-500'}`}>{conv.lastMessage}</p>
                            </div>
                            {conv.unreadCount > 0 && (
            ...
            {/* RIGHT: Chat Area */}
            {activeConversation ? (
                <div className={`flex-1 flex flex-col h-full bg-[#0B1121] ${!activeConvId ? 'hidden md:flex' : 'flex'}`}>
                    {/* Header */}
                    <div className="h-20 border-b border-white/5 flex items-center justify-between px-6 bg-[#0f1423]/50 backdrop-blur-sm z-10">
                        <div className="flex items-center gap-4">
                            <button onClick={() => setActiveConvId(null)} className="md:hidden text-slate-400 hover:text-white">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                            </button>
                            <div className="relative">
                                <img src={activeConversation.userAvatar} className="w-10 h-10 rounded-full border border-white/10" />
                            ...
                            <div>
                                <h3 className="font-bold text-white text-base leading-tight">{activeConversation.userName}</h3>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button 
                                onClick={onStartCall}
                                className="p-2.5 text-slate-400 hover:text-white hover:bg-white/5 rounded-xl transition-colors group"
                    ...
                    {/* Messages List */}
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6 bg-[#0B1121]">
                        <div className="flex justify-center">
                            <span className="text-[10px] font-bold text-slate-500 bg-white/5 px-3 py-1 rounded-full uppercase tracking-wider">Today</span>
                        </div>
                    ...
                    <div className="p-6 bg-[#0B1121] border-t border-white/5">
                        <form onSubmit={handleSendMessage} className="flex gap-4 items-end bg-[#1e293b] p-2 rounded-2xl border border-white/5 focus-within:border-cad-accent/50 transition-colors">
                            ...
                            <textarea 
                                ...
                                className="flex-1 bg-transparent border-none text-white placeholder-slate-500 focus:outline-none py-3 min-h-[48px] max-h-[120px] resize-none custom-scrollbar"
                            />
                ...
                <div className="flex-1 hidden md:flex flex-col items-center justify-center text-slate-500 bg-[#0B1121]">
