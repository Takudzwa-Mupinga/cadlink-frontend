...
const ResumeBuilder: React.FC<ResumeBuilderProps> = ({ initialData, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-lg flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="w-full max-w-6xl h-[90vh] bg-[#0f172a] rounded-3xl border border-white/10 shadow-2xl flex overflow-hidden">
                ...
                <div className="w-full lg:w-1/2 flex flex-col border-r border-white/10">
                    ...
                    <div className="flex bg-[#0f172a] border-b border-white/10">
                        <button onClick={() => setActiveSection('basics')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeSection === 'basics' ? 'border-cad-accent text-white' : 'border-transparent text-slate-500 hover:text-white'}`}>Basics</button>
                        <button onClick={() => setActiveSection('summary')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeSection === 'summary' ? 'border-cad-accent text-white' : 'border-transparent text-slate-500 hover:text-white'}`}>Summary</button>
                        <button onClick={() => setActiveSection('experience')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${activeSection === 'experience' ? 'border-cad-accent text-white' : 'border-transparent text-slate-500 hover:text-white'}`}>Experience</button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar bg-[#0f172a]">
                            ...
                            <div className="space-y-4 animate-in slide-in-from-left-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" value={resume.fullName} onChange={e => setResume({...resume, fullName: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cad-accent outline-none" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 mb-1 block">Job Title</label>
                                        <input type="text" value={resume.title} onChange={e => setResume({...resume, title: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cad-accent outline-none" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 mb-1 block">Email</label>
                                        <input type="text" value={resume.email} onChange={e => setResume({...resume, email: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cad-accent outline-none" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 mb-1 block">Location</label>
                                        <input type="text" value={resume.location} onChange={e => setResume({...resume, location: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-cad-accent outline-none" />
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="space-y-4 animate-in slide-in-from-left-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <button 
                                        ...
                                        className="text-xs font-bold text-cad-accent flex items-center gap-1 hover:text-white transition-colors"
                                    >
                                ...
                                <textarea 
                                    ...
                                    className="w-full bg-black/20 border border-white/10 rounded-xl p-4 text-white focus:border-cad-accent outline-none leading-relaxed"
                                />
                            </div>
                        )}
                        ...
                        {activeSection === 'experience' && (
                            <div className="space-y-6 animate-in slide-in-from-left-4">
                                {resume.experience.map((exp, idx) => (
                                    <div key={exp.id} className="bg-white/5 p-4 rounded-xl border border-white/5">
                                        <div className="grid grid-cols-2 gap-3 mb-3">
                                            ...
                                            }} className="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-bold" placeholder="Role" />
                                            ...
                                            }} className="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="Company" />
                                        </div>
                                        <div className="relative">
                                            <textarea 
                                                ...
                                                className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-sm text-slate-300 focus:border-cad-accent outline-none"
                                            />
                                            <button 
                                ...
                                <button className="w-full py-3 border border-dashed border-white/10 rounded-xl text-slate-500 hover:text-white hover:border-white/30 transition-colors text-sm font-bold flex items-center justify-center gap-2">
