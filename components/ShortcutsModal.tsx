...
const ShortcutsModal: React.FC<ShortcutsModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
            <div className="w-full max-w-lg bg-[#0f172a] rounded-3xl border border-white/10 shadow-2xl p-8 relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-2">
                    {shortcuts.map((s, i) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors group">
                            <span className="text-slate-300 font-medium group-hover:text-white transition-colors">{s.label}</span>
                            <div className="flex gap-1.5">
                                {s.key.map((k, idx) => (
                                    <kbd key={idx} className="bg-black/40 border border-white/10 px-2.5 py-1 rounded-lg text-xs font-bold text-slate-400 font-mono shadow-sm min-w-[28px] text-center">
                                        {k}
                                    </kbd>
                                ))}
                            </div>
