...
const ControlBtn: React.FC<ControlBtnProps> = ({ isActive, onClick, iconOn, iconOff, activeColor, inactiveColor, badge, label }) => (
    ...
    >
        ...
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-cad-border shadow-lg transform translate-y-2 group-hover:translate-y-0">
            {label}
        </div>
    </button>
);
...
const Studio: React.FC<StudioProps> = ({ initialMode = 'files' }) => {
  ...
  return (
    <div className={`h-full flex flex-col md:flex-row gap-4 max-w-full overflow-hidden bg-cad-dark ${isZenMode ? 'fixed inset-0 z-[100] p-0' : 'pt-4 pb-4 pr-4 pl-0 md:pl-0'}`}>
        
        {/* Zen Mode Exit Button */}
        {isZenMode && (
            <button 
                onClick={toggleZenMode}
                className="fixed top-4 right-4 z-[110] bg-cad-surface/60 hover:bg-black/80 text-cad-text p-2 rounded-full backdrop-blur-md border border-cad-border transition-colors"
                title="Exit Zen Mode"
            >
        ...
        {!isZenMode && (
            <div className="w-20 md:w-64 glass-panel rounded-r-2xl md:rounded-2xl flex flex-col hidden md:flex ml-4 overflow-hidden border border-cad-border shadow-premium">
                <div className="p-4 border-b border-cad-border bg-cad-panel">
                    <h3 className="font-bold text-cad-text text-xs uppercase tracking-widest text-center md:text-left text-cad-muted">Workspace</h3>
                </div>
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                                ...
                                : 'text-slate-400 hover:text-cad-text hover:bg-cad-surface/30 border border-transparent'
                            }`}
                        >
        ...
        {/* Main Workspace Area */}
        <div className={`flex-1 glass-panel flex flex-col overflow-hidden relative border border-cad-border shadow-2xl bg-cad-dark ${isZenMode ? 'rounded-none border-0' : 'rounded-2xl mr-4'}`}>
            ...
            {activeMode === 'files' && (
                ...
                <div className="flex-1 relative flex flex-col">
                    <div className="flex-1 flex relative">
                        {!isZenMode && (
                            <div className="w-60 border-r border-cad-border bg-cad-dark flex flex-col z-10 hidden lg:flex">
                                <div className="p-4 border-b border-cad-border flex justify-between items-center bg-cad-panel">
                                    <span className="text-xs font-bold text-cad-text uppercase tracking-wider flex items-center gap-2"><Layers className="w-4 h-4"/> Layers</span>
                                    <Plus className="w-4 h-4 text-slate-500 cursor-pointer hover:text-cad-text" />
                                </div>
                                <div className="p-2 space-y-1">
                                    {layers.map(l => (
                                        <div 
                                            ...
                                            className="flex items-center justify-between p-2.5 rounded-lg hover:bg-cad-surface/30 text-sm text-slate-300 cursor-pointer transition-colors"
                                        >
                                            <div className="flex items-center gap-3">
                                                <Eye className={`w-4 h-4 ${l.visible ? l.color : 'text-slate-700'}`} />
                                                <span className={l.visible ? 'text-cad-text font-medium' : 'text-slate-600'}>{l.name}</span>
                                            </div>
                        ...
                        <div className="flex-1 bg-cad-panel relative overflow-hidden group" onMouseMove={handleViewportMouseMove}>
                            ...
                            <div className="absolute top-4 left-4 flex gap-4 pointer-events-none">
                                <div className="bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs font-mono text-slate-300 border border-cad-border flex items-center gap-2">
                            ...
                            <div className="absolute bottom-28 left-1/2 -translate-x-1/2 glass-panel px-2 py-2 rounded-xl flex gap-1 border border-cad-border shadow-premium z-30">
                                {[ { id: 'select', icon: MousePointer2 }, { id: 'pan', icon: Move }, { id: 'orbit', icon: RotateCcw }, { id: 'zoom', icon: ZoomIn }, { id: 'measure', icon: Ruler } ].map(tool => (
                                    <button key={tool.id} onClick={() => setViewTool(tool.id)} className={`p-2.5 rounded-lg transition-all ${viewTool === tool.id ? 'bg-cad-accent text-cad-dark' : 'text-slate-400 hover:text-cad-text hover:bg-cad-surface/50'}`}>
                                        <tool.icon className="w-5 h-5" />
                                    </button>
                                ))}
                                <div className="w-px h-10 bg-cad-surface/50 mx-1"></div>
                                <button onClick={toggleZenMode} className={`p-2.5 rounded-lg transition-all ${isZenMode ? 'bg-cad-accent text-cad-dark' : 'text-slate-400 hover:text-cad-text hover:bg-cad-surface/50'}`}>
                                    {isZenMode ? <Minimize className="w-5 h-5" /> : <Maximize className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>
                    </div>
                    <div className="h-8 bg-cad-dark border-t border-cad-border flex items-center justify-between px-4 text-[10px] font-mono text-slate-400 select-none z-50">
                        <div className="flex items-center gap-4">
                            <span className="font-bold text-cad-accent">READY</span>
                            <span className="w-px h-3 bg-cad-surface/50"></span>
                    ...
                    {!meetingState.hasJoined ? (
                        ...
                        <div className="flex-1 flex items-center justify-center relative p-6">
                            ...
                            <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                                <div className="glass-panel p-1 rounded-3xl border border-cad-border shadow-2xl bg-black/40">
                                    <div className="bg-cad-surface/60 rounded-[20px] aspect-video relative overflow-hidden flex items-center justify-center group">
                                        {meetingState.camera ? (
                                            <>
                                                <img src="https://picsum.photos/800/600?random=99" className="w-full h-full object-cover opacity-80" alt="Preview" />
                                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold text-white border border-cad-border flex items-center gap-2">
                                                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> Camera On
                                                </div>
                                            </>
                                        ) : (
                                            <div className="flex flex-col items-center gap-4 text-slate-500">
                                                <div className="w-24 h-24 rounded-full bg-cad-surface/30 flex items-center justify-center">
                                        ...
                                        <div className="absolute bottom-4 right-4 flex gap-2">
                                            <button 
                                                ...
                                                className={`p-3 rounded-full transition-all ${meetingState.mic ? 'bg-white text-black hover:bg-slate-200' : 'bg-red-500 text-cad-text'}`}
                                            >
                                                {meetingState.mic ? <Mic className="w-5 h-5"/> : <MicOff className="w-5 h-5"/>}
                                            </button>
                                            <button 
                                                onClick={() => toggleMeetingState('camera')}
                                                className={`p-3 rounded-full transition-all ${meetingState.camera ? 'bg-white text-black hover:bg-slate-200' : 'bg-red-500 text-cad-text'}`}
                                            >
                                ...
                                <div className="flex flex-col justify-center space-y-6">
                                    <div>
                                        <h2 className="text-3xl font-bold text-cad-text mb-2">Ready to join?</h2>
                                        <p className="text-slate-400">HVAC Design Review • 3 Participants Waiting</p>
                                    </div>

                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between p-4 bg-cad-surface/30 rounded-2xl border border-cad-border">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-purple-500/20 flex items-center justify-center text-purple-400">
                                                    <Users className="w-5 h-5" />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-cad-text">Sarah, Mike, Elena</p>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <button className="px-6 py-4 bg-cad-surface/30 text-cad-text rounded-xl font-bold hover:bg-cad-surface/50 transition-colors border border-cad-border">
                                            Present
                                        </button>
                                    </div>
                                </div>
                        ...
                        <>
                            ...
                            <div className="absolute top-6 left-6 z-20 flex items-center gap-4">
                                <div className="glass-panel px-4 py-2 rounded-full flex items-center gap-3 border border-cad-border bg-black/40 backdrop-blur-xl">
                                    <div className="flex items-center gap-2">
                                        <span className="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.5)]"></span>
                                        <span className="text-xs font-bold text-cad-text uppercase tracking-wider">REC</span>
                                    </div>
                                    <div className="w-px h-4 bg-cad-surface/50"></div>
                                    <span className="text-sm font-bold text-slate-200">HVAC Design Review</span>
                                    <span className="px-1.5 py-0.5 rounded bg-cad-surface/50 text-[10px] font-mono text-slate-400">00:12:45</span>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 flex relative">
                                <div className={`flex-1 p-6 grid gap-6 transition-all duration-300 ${meetingState.screenShare ? 'grid-cols-4 grid-rows-4' : 'grid-cols-2 md:grid-cols-2 lg:grid-cols-3'} content-center`}>
                                    ...
                                    {meetingState.screenShare && (
                                        <div className="col-span-3 row-span-4 bg-cad-surface rounded-3xl border border-cad-accent/30 relative overflow-hidden shadow-2xl group ring-1 ring-white/10">
                                            <div className="h-8 bg-cad-dark border-b border-cad-border flex items-center px-4 justify-between">
                                            ...
                                            <div className="absolute bottom-6 left-6 bg-black/60 backdrop-blur-md px-4 py-2 rounded-xl border border-cad-border flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-cad-accent flex items-center justify-center text-cad-dark font-bold text-xs">YO</div>
                                                <div>
                                                    <p className="text-xs font-bold text-cad-text leading-tight">You are presenting</p>
                                                    <p className="text-[10px] text-green-400 font-medium">Screen Share Active</p>
                                                </div>
                                                <button onClick={() => toggleMeetingState('screenShare')} className="ml-2 p-1.5 hover:bg-cad-surface/50 rounded-lg text-cad-text"><X className="w-4 h-4"/></button>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {COLLABORATORS.map((user, idx) => {
                                        ...
                                        return (
                                            <div key={user.id} className={`relative bg-cad-surface rounded-3xl overflow-hidden border shadow-lg group transition-all ${
                                                user.id === 'client' ? 'border-green-500/50 shadow-[0_0_30px_rgba(34,197,94,0.15)]' : 'border-cad-border'
                                                ...
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent">
                                                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                                                        <div className="flex flex-col gap-1">
                                                            <span className="text-sm font-bold text-cad-text flex items-center gap-2 text-shadow-sm">
                                                                {user.name} {isMe && '(You)'}
                                                        ...
                                                        <div className="p-2 rounded-full bg-cad-surface/50 backdrop-blur-md border border-cad-border">
                                ...
                                {meetingState.showChat && (
                                    <div className="w-96 bg-cad-dark/95 backdrop-blur-xl border-l border-cad-border flex flex-col animate-in slide-in-from-right duration-300 z-30">
                                        <div className="h-16 border-b border-cad-border flex items-center justify-between px-6">
                                            <h3 className="font-bold text-cad-text">Meeting Chat</h3>
                                            <button onClick={() => toggleMeetingState('showChat')} className="p-2 hover:bg-cad-surface/30 rounded-full text-slate-400 hover:text-cad-text transition-colors"><X className="w-5 h-5"/></button>
                                        </div>
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                <div>
                                                    <div className="flex items-baseline gap-2">
                                                        <span className="text-xs font-bold text-cad-text">Sarah (Client)</span>
                                                        <span className="text-[10px] text-slate-500">10:42 AM</span>
                                                    </div>
                                                    <p className="text-sm text-slate-300 bg-cad-surface/30 p-3 rounded-2xl rounded-tl-none mt-1 border border-cad-border">Can we zoom in on the mechanical room layout?</p>
                                                </div>
                                            </div>
                                            <div className="flex gap-3 flex-row-reverse">
                                                ...
                                                <div className="text-right">
                                                    <div className="flex items-baseline gap-2 justify-end">
                                                        ...
                                                        <span className="text-xs font-bold text-cad-text">You</span>
                                                    </div>
                                                    <p className="text-sm text-cad-text bg-blue-600 p-3 rounded-2xl rounded-tr-none mt-1 shadow-lg shadow-blue-900/20">Sure, I'll switch to the 3D view.</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="p-4 border-t border-cad-border">
                                            <div className="relative">
                                                <input type="text" placeholder="Send a message..." className="w-full bg-black/40 border border-cad-border rounded-xl pl-4 pr-10 py-3 text-sm text-cad-text focus:border-cad-accent outline-none transition-colors" />
                            ...
                            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-40">
                                <div className="glass-panel px-4 py-3 rounded-2xl flex items-center gap-3 border border-cad-border shadow-2xl bg-cad-dark/80 backdrop-blur-2xl ring-1 ring-white/5 scale-100 hover:scale-105 transition-transform duration-300">
                                    <ControlBtn 
                                        isActive={meetingState.mic} 
                                        onClick={() => toggleMeetingState('mic')} 
                                        iconOn={<Mic className="w-5 h-5" />} 
                                        iconOff={<MicOff className="w-5 h-5" />}
                                        activeColor="bg-cad-surface/50 text-cad-text"
                                    ...
                                    <div className="w-px h-8 bg-cad-surface/50 mx-2"></div>
                                    ...
                                    <ControlBtn 
                                        ...
                                        activeColor="bg-green-500 text-cad-text shadow-lg shadow-green-500/20"
                                        inactiveColor="bg-cad-surface/50 text-cad-text hover:bg-cad-surface/70"
                                        label="Screen Share"
                                    />
                                    <ControlBtn 
                                    ...
                                    <button 
                                        onClick={() => toggleMeetingState('hasJoined')}
                                        className="bg-red-500 hover:bg-red-600 text-cad-text px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-red-900/40 active:scale-95 flex items-center gap-2 whitespace-nowrap"
                                    >
            ...
            {activeMode === 'board' && (
                <div className="flex-1 bg-cad-dark p-6 overflow-x-auto custom-scrollbar">
                    {/* ... (Existing Board content) ... */}
                    <div className="flex gap-6 h-full">
                        {['Todo', 'In Progress', 'Done'].map(status => (
                            <div key={status} className="w-80 flex-shrink-0 flex flex-col bg-cad-panel rounded-xl border border-cad-border">
                                <div className="p-4 border-b border-cad-border font-bold text-xs text-cad-muted uppercase tracking-widest">{status}</div>
                                <div className="p-4 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                                    {tasks.filter(t => t.status === status).map(t => (
                                        <div key={t.id} className="bg-cad-surface p-4 rounded-lg border border-cad-border shadow-sm hover:border-cad-accent/30 cursor-pointer transition-colors group">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${t.priority === 'High' ? 'text-red-400 border-red-900 bg-red-900/20' : 'text-slate-400 border-slate-700 bg-cad-surface'}`}>{t.priority}</span>
                                            </div>
                                            <p className="text-sm font-bold text-cad-text mb-3 group-hover:text-cad-accent transition-colors">{t.title}</p>
                                            <div className="flex items-center gap-2">
                                    ...
                                    <button className="w-full py-2 border border-dashed border-cad-border text-slate-500 text-xs font-bold rounded hover:border-cad-accent hover:text-cad-accent transition-colors">+ Add Task</button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {activeMode === 'chat' && (
                <div className="flex-1 flex flex-col bg-cad-dark">
                    <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                        {messages.map(msg => (
                            <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[70%] p-4 rounded-2xl text-sm leading-relaxed ${msg.sender === 'me' ? 'bg-cad-accent text-cad-dark' : 'bg-cad-surface/30 text-cad-text border border-cad-border'}`}>
                                    {msg.text}
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="p-4 bg-cad-panel border-t border-cad-border">
                        <div className="flex gap-2">
                             <input 
                                className="flex-1 bg-cad-dark border border-cad-border rounded-xl px-4 py-3 text-cad-text focus:border-cad-accent outline-none"
            ...
            {activeMode === 'dream' && (
                <div className="flex-1 p-10 bg-cad-dark flex flex-col items-center justify-center">
                    <div className="max-w-2xl w-full space-y-8">
                        <div className="text-center">
                            ...
                            <h2 className="text-3xl font-bold text-cad-text mb-2">DreamCanvas</h2>
                        ...
                        <div className="glass-panel p-2 rounded-xl flex gap-2 border border-purple-500/20 shadow-[0_0_30px_rgba(168,85,247,0.1)]">
                            <input 
                                ...
                                className="flex-1 bg-transparent border-none px-4 text-cad-text focus:outline-none placeholder-slate-600"
                            />
                            <button 
                                onClick={handleGenerateImage}
                                disabled={isDreaming || !dreamPrompt}
                                className="px-6 py-3 bg-purple-600 hover:bg-purple-500 text-cad-text font-bold rounded-lg transition-all flex items-center gap-2"
                            >
                        ...
                        {generatedImage && (
                            <div className="rounded-xl overflow-hidden border border-cad-border shadow-2xl animate-in zoom-in-95">
