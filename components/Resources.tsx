    ...
    return (
        <div className="h-full overflow-y-auto custom-scrollbar p-6 md:p-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-cad-text flex items-center gap-2">
            ...
            <div className="glass-panel p-2 rounded-2xl flex flex-col md:flex-row gap-3 items-center sticky top-4 z-20 shadow-xl">
                <div className="relative flex-1 w-full md:w-auto">
                    ...
                    <input 
                        ...
                        className="w-full bg-transparent border-none rounded-xl pl-10 pr-4 py-2.5 text-cad-text focus:outline-none placeholder-slate-500 font-medium"
                    />
                </div>
                <div className="w-px h-8 bg-cad-surface/50 hidden md:block"></div>
                    ...
                    {['All', '2D Block', '3D Model', 'Material'].map((tab) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${
                                ...
                                : 'text-slate-400 hover:text-cad-text hover:bg-cad-surface/30'
                            }`}
                        >
                            {tab}
                        </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAssets.map(asset => (
                    <div key={asset.id} className="glass-card rounded-2xl overflow-hidden hover:border-cad-accent/40 hover:shadow-glow transition-all duration-300 group hover:-translate-y-1 flex flex-col">
                        <div className="relative h-56 bg-cad-surface overflow-hidden">
                            ...
                            <div className="absolute top-3 left-3 flex gap-2">
                                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider text-cad-text border border-cad-border backdrop-blur-md shadow-lg ${
                        ...
                        <div className="p-6 flex flex-col flex-1 relative -mt-4">
                            <div className="flex justify-between items-start mb-3">
                                <h3 className="font-bold text-cad-text text-lg leading-tight group-hover:text-cad-accent transition-colors">{asset.title}</h3>
                            </div>
                            
                            <div className="flex items-center gap-4 text-xs text-slate-400 mb-6 bg-cad-surface/30 p-3 rounded-xl border border-cad-border">
                            ...
                            <div className="mt-auto">
                                <button 
                                    ...
                                    className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all duration-300 ${
                                        ...
                                        ? 'bg-green-500 text-cad-text shadow-lg' 
                                        : 'bg-cad-surface/30 hover:bg-cad-accent hover:text-cad-dark text-cad-text border border-cad-border hover:border-transparent hover:shadow-glow'
                                    }`}
                                >
                                    {downloading === asset.id ? (
                                        <>
